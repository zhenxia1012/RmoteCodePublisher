  ...
  public:
    bool doTest(const Scanner::ITokCollection* pTc) override
    {
      ...
      if (tc.length() > 0 && tc[0] == "using"){
        doActions(pTc);
        return IRule::Stop;}
	  /*if (tc.length() > 0 && tc[0] == "typedef"){
		  doActions(pTc);
		  return IRule::Stop;}
	  if (tc.length() > 0 && tc[0] == "enum"){
		  doActions(pTc);
		  return IRule::Stop;}*/
      std::string debug = tc.show();  
      if (tc[tc.length() - 1] == ";" && tc.length() > 2){
        std::string nextToLast = tc[tc.length() - 2];
        if (nextToLast == "delete" || nextToLast == "default" || nextToLast == "const"){{// function declaration
            doActions(pTc);
            return IRule::Stop;}}
        std::string parentType = pRepo->scopeStack().top()->type_;   
