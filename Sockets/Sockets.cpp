...
{
  int bytesSent = 0, bytesLeft = static_cast<int>(bytes);
  byte* pBuf = buffer;
  while (bytesLeft > 0)
  {
...
{
  int bytesRecvd = 0, bytesLeft = static_cast<int>(bytes);
  byte* pBuf = buffer;
  while (bytesLeft > 0)
  {
...
{
  int bytesSent, bytesRemaining = static_cast<int>(str.size());
...
{
  return ::send(socket_, pBuf, static_cast<int>(bytes), 0);
}
...
{
  return ::recv(socket_, pBuf, static_cast<int>(bytes), 0);
}
