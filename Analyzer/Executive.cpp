...
int main(int argc, char* argv[])
{
	...
	try {
		...
		Rslt::write("\n");
		exec.stopLogger();
		std::cout << "\n  Code Analysis completed";Utilities::putline();
		Utils::Title("Initialize Dependency Relationship");Utilities::putline();
		NoSqlDb<std::string> db("Project3 Dependency Database");
		Requirement req_;req_.iniDp(db);
		Utils::Title("Demonstration for Project3 - CodePublisher");req_.requirment(db);}
